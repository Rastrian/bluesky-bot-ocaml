...
let unfollow token did handle =
  Logs.info (fun m -> m "Unfollowing user: %s" handle);
  let headers = Cohttp.Header.of_list [("Authorization", "Bearer " ^ token); ("Content-Type", "application/json")] in
  let body = `Assoc [("subject", `String handle); ("repo", `String did)] |> to_string in

  Cohttp_lwt_unix.Client.post ~headers ~body:(Cohttp_lwt.Body.of_string body) (Uri.of_string (api_url ^ "/app.bsky.graph.follow/delete"))
  >>= fun (resp, body) ->
  Cohttp_lwt.Body.to_string body >|= fun body_str ->
  Logs.debug (fun m -> m "Response for unfollow user %s: %s" handle body_str);
  if Cohttp.Response.status resp |> Cohttp.Code.code_of_status = 200 then
    Logs.info (fun m -> m "Successfully unfollowed %s." handle)
  else
    Logs.err (fun m -> m "Failed to unfollow %s. Response: %s" handle body_str)
